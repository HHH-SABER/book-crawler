...
### 「站点管理」配置与「适配器插件」的分工（v2.4.28 定稿）

GUI「站点管理」页**同时展示两层**，二者互补而非重复，都是一套站点适配体系的入口：

| 维度 | 站点配置（JSON / 内置 SITE_PATTERNS） | 适配器插件（站点适配/*.py） |
|---|---|---|
| 形式 | 数据（JSON 规则，可 GUI 增删改） | 代码（插件，可 GUI 新建/删除/打开目录） |
| 适用 | 纯选择器、参数化站点（选择器/分页/反爬全字段可配） | 需自定义逻辑的站（加密正文、特殊目录、AJAX 两步、特殊分页） |
| 生效 | 改配置即生效，**免改代码** | 放目录即生效，**免重新打包** |
| 优先级 | 底层基础配置 | 插件命中则**优先于**内置/JSON 规则 |
| 健康度 | 状态列 = 爬取历史成功率 + 反爬命中 + 探测结果 | 插件列表显示 目录/正文/分页 能力徽标 |

结论：**站点管理页仍然有用** —— 它是"全量站点适配的统一视窗"：JSON 规则管理
站点级参数与健康度监控，插件区管理代码级适配；两者在同一页，恰好让用户看清
"普通站走配置、怪站走插件"的分层。故 v2.4.28 不做界面重排，保留现状并补此说明
（避免"有了插件就没必要管 JSON 配置"的误解）。

> 判断建议：新站先试 JSON 规则（方式 A，成本最低）；JSON 表达能力不足
> （加密/动态加载/特殊目录结构）再转插件；两者都可从「站点管理」页完成，无需动代码。
