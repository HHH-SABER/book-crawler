...
- **阴性路径**：victor 清单 URL 遭 GitHub Pages 边缘缓存抖动（同 URL 200/404
  交替，多次探活 404 面）——程序在 404 假页上**干净终止**（0 章→提示→退出码 0、
  无残留、无 Traceback），站点不可达场景的稳健性得到真实验证。

### 自愈 — 批3 PoC-C：站点管理页内嵌审核卡（GUI 采纳/拒绝）

- `site_manage_page` 新增"选择器自愈建议"卡（适配器卡与表格之间）：逐条显示
  域名/建议选择器/容器规模/置信度（≥0.85 绿、≥0.7 琥珀、其余红），行内 ✔/✘
  按钮 = 后台线程调 `采纳建议`/`拒绝建议` 核心函数（与 CLI 同一入口），完成后
  `_dispatch` 回主线程刷新；采纳成功即重读配置重建站点表格，新选择器立即可见。
- 高频安全：`refresh()` 每秒调用，渲染走 `(mtime,size)` 签名比对 + 控件空判定，
  未变化零读盘零重建；`_heal_busy` 集合防双击重复并入；按钮 `disabled` 置灰。
- 无头冒烟（真实组件不启窗口）：build 渲染 → 注入建议 → 采纳 → 临时副本配置
  `content_selectors` 前插生效 + 建议清空 ✔；语法/静态/326 tests/换行 CRLF 保持。
...
- **正确形态=加密变更线索**：`选择器自愈.加密变更线索(html, domain)` 在 qsbs
  分支提取 ≤100 字时探测页面上"大 Base64 实参调用"中未被已知解密链覆盖的
  函数名（白名单 `qsbs.bb/str_decode/atob/.bb 后缀/writeln 包裹`），产出
  **类型=加密变更** 的线索建议：无配置可改故**不提供一键采纳**（GUI 行只给
  忽略按钮 + 琥珀警示图标；CLI 标注"需人工适配"），处理 = 扩展 decrypt_utils
